...
import com.letters7.wuchen.springboot2.utils.datetime.UtilDate;
...
public class UtilClass {
    ...
    public static final <T> T cast(Object value, Class<T> type) {
        ...
        Object newVal = null;
        if (UtilClass.instanceOf(value.getClass(), type)) {
            newVal = value;
        } else if (type == Integer.class || type == int.class) {
            newVal = UtilString.toInt(value);
        } else if (type == Double.class || type == double.class || type == Float.class || type == float.class) {
            newVal = UtilString.toDouble(value);
        } else if (type == Long.class || type == long.class) {
            newVal = UtilString.toLong(value);
        } else if (type == Boolean.class || type == boolean.class) {
            newVal = UtilString.isTrue(value);
        } else if (type == Date.class) {
            newVal = UtilDate.parseDateTime(value.toString());
        } else if (type == String.class) {
            if (value instanceof Date) {
                newVal = UtilDate.formatDateTime(((Date) value), UtilDate.DATE_TIME_FORMAT);
            } else {
